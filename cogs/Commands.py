...
class Commands(commands.Cog):
    ...
    @commands.command(description="Returns information about a specific episode", usage="[episode]")
    async def episode(self, ctx, *, episode="None"):
        async with aiohttp.ClientSession() as session:
            try:
                ...
                pass
            async with session.get(f'http://127.0.0.1:5000/episode/{episode.lower()}') as response:
                try:
                    response = await response.json()
